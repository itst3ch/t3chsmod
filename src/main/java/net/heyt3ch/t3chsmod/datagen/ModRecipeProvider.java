        ...
        for(Item item : Registries.ITEM)
        {
            if (item instanceof Equipment && EnchantmentTarget.ARMOR.isAcceptableItem(item))
            {
                ...
                SmithingTransformRecipeJsonBuilder.create(Ingredient.ofItems(ModdedItems.HALITE_UPGRADE), Ingredient.ofItems(item), Ingredient.ofItems(ModdedItems.HALITE_CRYSTAL), RecipeCategory.COMBAT, Items.ACACIA_BOAT)
                        .criterion(FabricRecipeProvider.hasItem(ModdedItems.HALITE_CRYSTAL), FabricRecipeProvider.conditionsFromItem(ModdedItems.HALITE_CRYSTAL)).offerTo(exporter, recipeId);
